...
const register = asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    return res.status(200).json({
      ...
      mess: newUser ? "create user successfully" : "something went wrong",
    });
  }
});
...
const login = asyncHandler(async (req, res) => {
  ...
  if (response && (await response.isCorrectPassword(req.body.password))) {
    ...
    let newRefreshToken;
    if (remember) {
      newRefreshToken = generateRefreshToken(response._id);
    }
    // console.log(newRefreshToken);
    // Update lai refreshToken vao database - new : true la tra ve data sau khi update
    await User.findByIdAndUpdate(
      response._id,
      { refreshToken: remember ? newRefreshToken : null },
      { new: true }
    );
...
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  if (!email) {
    throw new Error("Missing email! Please try again.");
  }
  ...
  // console.log(user.createPasswordChangedToken())
  const resetToken = user.createPasswordChangedToken();

  await user.save(); // luu vao database

  const html = `Xin vui long click vao link duoi day de thay doi mat khau cua ban.Link nay se het han sau 15 phut ke tu bay gio. <a href=${process.env.URL_SERVER}/reset-password/${resetToken}>Click here</a>`;
